...
megagrid <- expand.grid(ID = unique(m0.df$ID), 
                        ...
                        Period = c("00", "02", "12")) 
megagrid$MaxRL[which(megagrid$Period == "00" | megagrid$Period == "02")] <- 0

megagrid <- left_join(m0.df[,c(1,5,6)], megagrid, by = "ID") %>%
  distinct() 

megagrid$Sonar <- 0
megagrid$Sonar[which(megagrid$Period=="12")] <- 1

megagrid$m0.pred <- predict.gam(m0, newdata=megagrid, type = "response")

presonar <- filter(megagrid, Period != 12) %>%
  pivot_wider(id_cols = c(ID), names_from = Period, values_from = c(m0.pred))
withsonar <- filter(megagrid, Period == "12")
m0.results <- left_join(withsonar, presonar, by = "ID")

m0.results$delta31 <- (m0.results$m0.pred-m0.results$`00`)/m0.results$`00`
m0.results$delta32 <- (m0.results$m0.pred-m0.results$`02`)/m0.results$`02`
m0.results$delta21 <- (m0.results$`02`-m0.results$`00`)/m0.results$`00`
...
for (i in 1:1000){
  ...
  megagrid$m0.exp <- m0.gam$family$linkinv(megagrid$m0.pred)
  
  presonar <- filter(megagrid, Period != 12) %>%
    pivot_wider(id_cols = c(ID), names_from = Period, values_from = c(m0.pred, m0.exp))
  withsonar <- filter(megagrid, Period == "12")
  ...
  m0.bs.results$delta21 <- (m0.bs.results$m0.exp_02-m0.bs.results$m0.exp_00)/m0.bs.results$m0.exp_00
  m0.bs.results$delta31 <- (m0.bs.results$m0.exp_12-m0.bs.results$m0.exp_00)/m0.bs.results$m0.exp_00
  m0.bs.results$delta32 <- (m0.bs.results$m0.exp_12-m0.bs.results$m0.exp_02)/m0.bs.results$m0.exp_02
...
m0.bootstrap.df <- do.call(rbind.data.frame, m0.bs)
...
ggsave(plot = last_plot(), filename = "./Figures/M0.png", width = 6, height = 4, units = "in")
